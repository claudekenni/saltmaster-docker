...
number_masters=$(docker-compose ps | grep docker_master | grep Up | wc -l)
...
do 
   masters[${i}]="$(docker container inspect -f '{{ .NetworkSettings.Networks.saltmaster_docker_salt_net.IPAddress }}' saltmaster_docker_master_${i})"
